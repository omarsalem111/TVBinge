...
export async function searchShowByName(searchQuery) {
  ...
  searchResults.results.forEach(
    (show) => (show.poster_path = baseUrl + posterSizes[6] + show.poster_path),
  );
...
export async function fetchShowDetails(id) {
  ...
  customShowData.poster_path =
    baseUrl + posterSizes[6] + customShowData.poster_path;
  ...
  customShowData.seasons.forEach(
    (season) =>
      (season.poster_path = baseUrl + posterSizes[6] + season.poster_path),
  );
...
export async function fetchPopularShows(pageNum, filters) {
  ...
  const editedShows = { ...shows };

  editedShows.results.forEach(
    (show) => (show.poster_path = baseUrl + posterSizes[6] + show.poster_path),
  );

  console.log("Printed at line 134 TMDB.js", editedShows);
  return editedShows;
}

export async function mapFilters(filters, searchParams) {
...
export async function fetchEpisodeDetails(id, season, episode) {
  ...
  const episodeData = await fetchAndParse(
    `https://api.themoviedb.org/3/tv/${id}/season/${season}/episode/${episode}?append_to_response=credits&language=en-US`,
  );
  const customEpisodeData = { ...episodeData };

  delete customEpisodeData.crew;

  delete customEpisodeData.guest_stars;
  ...
  customEpisodeData.logo_path =
    imagesData.logos[0] &&
    baseUrl + posterSizes[3] + imagesData.logos[0].file_path;

  customEpisodeData.still_path =
    baseUrl + posterSizes[6] + customEpisodeData.still_path;

  const creditsArray = Object.values(customEpisodeData.credits);

  creditsArray.forEach((subArray) =>
    subArray.forEach(
      (creditsObject) =>
        (creditsObject.profile_path =
          baseUrl + posterSizes[6] + creditsObject.profile_path),
    ),
  );

  customEpisodeData.number_of_seasons = showData.number_of_seasons;

  customEpisodeData.season_episodes_count = showData.seasons.filter(
    (seasonObject) => seasonObject.season_number === parseInt(season),
  )[0].episode_count;

  customEpisodeData.next_season = showData.seasons.filter(
    (seasonObject) => seasonObject.season_number === parseInt(season) + 1,
  )[0];

  customEpisodeData.refShowData = {
    id: showData.id,
    name: showData.name,
  ...
  showData.seasons.forEach(
    (season) =>
      (customEpisodeData.refShowData.episodeCount += season.episode_count),
  );

  // console.log(customEpisodeData);

  return customEpisodeData;
}
...
export async function fetchSeasonEpisodes(id, season) {
  ...
  const customEpisodes = { ...episodes };

  customEpisodes.episodes.forEach(
    (episode) =>
      (episode.still_path = baseUrl + posterSizes[6] + episode.still_path),
  );

  console.log(
    "Printed at function fetchSeasonEpisodes TMDB.js",
    customEpisodes,
  );
